...
class integrations_test(unittest.TestCase):
    ...
    def test_update_merchant(self):
        ...
        result = ai.update_merchant(self.token, self.url, f'/api/merchants/{richard_id}', merch_data)
        self.assertEqual(merch_data, result)

    def test_delete_merchant(self):
        beauty_id = '9001976c-a9e7-4b95-b133-9ac8ba213fb2'
        result = ai.delete_merchant(self.token, self.url, f'/api/merchants/{beauty_id}')
    ...
    def test_post_product(self):
        ...
        result = ai.post_products(self.token, self.url, richard_id, df, 0, len(df))
    ...
    def test_join_columns(self):
        ...
        df = pd.DataFrame(data=products)
        df = ci.join_df_columns(df, 'CATEGORY', ['SUBCATEGORY', 'SUBSUBCATEGORY'], '|')
        package = df['CATEGORY'].tolist()
        self.assertEqual(package, ['snacks|salados|papas','bebestibles|gaseosas|cola'])
